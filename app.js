...
async function confirmDeleteCourseWithCode(course) {
  ...
  while (true) {
    const answer = await confirmCodeDialog(
      '删除球场',
      course.editCode
        ? '输入球场编辑密码，然后选择“是”以删除此球场。'
        : '此旧球场没有编辑密码，请使用通用密码删除。',
      errorMessage
    );
    if (answer === false) return false;
...
function addListeners() {
  ...
  els.courseForm.addEventListener('submit', async event => {
    ...
    if (!valid) {
      ...
      target.setCustomValidity(!name ? '请输入球场名称。' : (!/^\d{2}$/.test(editCode) ? '请输入两位数密码。' : '请输入1至7之间的标准杆数。'));
  ...
  els.languageButton.addEventListener('click', () => {
    window.location.assign('https://zxj088.github.io/golf-score-vegas/');
  });
}
